...
void runChild(struct execConfig *execConfig) {
    ...
    if (execConfig->stdinPath[0] != '\0') {
        inputFile = fopen(execConfig->stdinPath.c_str(), "r");
        if (!inputFile) {
    ...
    if (execConfig->stdoutPath[0] != '\0') {
        outputFile = fopen(execConfig->stdoutPath.c_str(), "w");
        if (!outputFile) {
    ...
    if (execConfig->stderrPath[0] != '\0') {
        errFile = fopen(execConfig->stderrPath.c_str(), "w");
        if (!errFile) {
            CHILD_EXIT(CAN_NOT_MAKE_OUTPUT);
        }
    ...
    // 执行用户的提交
    execve(execConfig->execPath.c_str(), NULL, envp);
