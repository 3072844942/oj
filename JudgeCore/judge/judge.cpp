...
void runJudge(struct execConfig *execConfig, struct judgeResult *judgeResult) {
    ...
    int current = 1;
    // 如果文件存在
    while (exists(execConfig->stdinPath + std::to_string(current) + ".in")) {
        struct execConfig childConfig;
        struct judgeResult childResult;

        childConfig.execPath = execConfig->execPath;
        childConfig.stdoutPath = execConfig->stdoutPath + std::to_string(current) + ".out";
        childConfig.cpuTimeLimit = execConfig->cpuTimeLimit;
        childConfig.memoryLimit = execConfig->memoryLimit;
        childConfig.stdinPath = execConfig->stdinPath + std::to_string(current) + ".in";

        // 进行子用例判断
        runForSingleJudge(&childConfig, &childResult);

        // cpu时间 注意和真实时间区分
        judgeResult->cpuTimeCost = std::max(judgeResult->cpuTimeCost, childResult.cpuTimeCost);
        //WARNING：该值在mac和linux系统上是有区别的！
        judgeResult->memoryCost = std::max(judgeResult->memoryCost, childResult.memoryCost);
        // 子用例运行失败
        if (childResult.condition != RUN_SUCCESS) {
            judgeResult->condition = childResult.condition;
    ...
    // 判题成功
    judgeResult->condition = RUN_SUCCESS;
    gettimeofday(&endTime, NULL);
    judgeResult->realTimeCost = getGapMillsecond(startTime, endTime);
}

void runForSingleJudge(struct execConfig *execConfig, struct judgeResult *judgeResult) {
    struct timeval startTime, endTime;
